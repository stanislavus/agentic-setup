...
1. **Native tools** (Read, Edit, Write) — for direct file manipulation in the workspace
2. **Context Mode** — for large output, research, and data processing
3. **Domain-specific MCP** — use the most specialized server for the task
4. **General-purpose fallback** — Filesystem, Shell, etc.
...
- Context Mode hooks are active (see `.claude/settings.json`). They auto-index tool output to protect the context window.
...
- Do NOT use `ctx_execute` to create, modify, or overwrite files. Use native Write/Edit tools instead.
- After `/clear` or `/compact`, the knowledge base is preserved. Use `ctx purge` to wipe it.
...
- Use Serena for intelligent, symbol-level code navigation instead of reading entire files.
...
- For files within the current workspace, prefer native tools (Read, Edit, Write) over Filesystem MCP.
...
- For complex git operations (merge, rebase), prefer running git CLI directly.
...
- Do NOT use Read, Grep, or any other tool to read `.pen` files. Always use Pencil MCP (`batch_get`, `batch_design`).
